...
watch(
  ...
  async (imageId) => {
    ...
    if (imageId && !draft.value.avatarPreview) {
      ...
      const matchedImage = savedImages.value.find(
        (img) => img.supabaseId === imageId
      );
      if (matchedImage) {
        applyDefaultImage(matchedImage);
        console.log(
          "Applied default image from preferences:",
          matchedImage.name
        );
      }
    }
...
async function handleUrlSubmit() {
  ...
  try {
    ...
    const response = await $fetch<Blob>(
      `/api/upload?url=${encodeURIComponent(imageUrl.value.trim())}`,
      {
        responseType: "blob",
      }
    );
...
<template>
  <div class="card p-4">
    ...
    <button
      v-else
      class="w-full aspect-square border-2 border-dashed border-stone-300 rounded-xl hover:border-purple-400 hover:bg-purple-50/50 transition-all flex flex-col items-center justify-center cursor-pointer bg-stone-50"
      @click="handleClick"
    >
      <CirclePlus class="w-10 h-10 mb-2 text-stone-400" />
      <p class="text-sm font-medium text-stone-500">新增照片</p>
    </button>
      ...
      >
        ...
        <div
          ...
          :class="[
            ...
            isPortrait
              ? 'h-[85vh] aspect-[9/16]'
              : 'w-[85vw] aspect-video max-h-[85vh]',
          ]"
          @click.stop
        >
          <!-- Ratio Toggle -->
          <div
            class="px-4 py-2 bg-stone-800 border-b border-stone-700 shrink-0"
          >
          ...
          >
            ...
            >
              <video v-if="cameraError" class="hidden" />
              <video
                ref="videoRef"
                autoplay
                playsinline
          ...
          <!-- Camera Controls -->
          <div
            class="p-4 bg-stone-900 flex justify-center items-center gap-6 shrink-0"
          >
        ...
        >
          ...
          <div class="flex-1 overflow-auto p-4">
              ...
              >
                ...
                >
                  ...
                  >
                    <svg
                      v-if="
                        deletingId === (image.supabaseId || String(image.id))
                      "
            ...
            <!-- Upload Tab -->
            <div v-else-if="modalTab === 'upload'" class="space-y-4">
              <!-- Drag & Drop Zone -->
              <div
              ...
              <!-- 黑色選擇檔案按鈕 -->
              <button
                class="w-full py-3 bg-stone-900 hover:bg-stone-800 text-white font-medium rounded-xl transition-colors flex items-center justify-center gap-2"
